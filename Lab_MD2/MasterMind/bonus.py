...
answer_colors = [random.choice(choice_colors), random.choice(choice_colors), random.choice(choice_colors), random.choice(choice_colors)]
...
class MainWidget(Widget):
    ...
    def redraw(self):
        global guess_colors, fd_colors, solved
        guess_colors = [[white, white, white, white],
                        [gray, gray, gray, gray],
                        [gray, gray, gray, gray],
                        [gray, gray, gray, gray],
    ...
    def init_bg(self):
        ...
        with self.canvas:
            ...
            for i in range(4):
                self.answers.append(Ellipse(bg_color=Color(answer_colors[i][0], answer_colors[i][1], answer_colors[i][2])))
            for i in range(10):
                for j in range(4):
            ...
            for i in range(6):
                self.choices.append(Ellipse(bg_color=Color(choice_colors[i][0], choice_colors[i][1], choice_colors[i][2])))
    ...
    def update_bg(self):
        ...
        for i in range(10):
            for j in range(4):
                ...
                self.feedback[i][j].pos = int((self.width/11) * col) + int(self.width/11 * .25), \
                    int((self.height/13) * (i+2)) + row * int((self.height/13) * 0.4) + int((self.height/13)*.08)
        ...
        if not solved and turn < 10:
            self.answer_cover.pos = int(self.width/5) + 3, int(self.height/13) * 12 + 3
            self.answer_cover.size = int(self.width/5 * 4) - 6, int(self.height/13) + 9
        else:
    ...
    def on_touch_down(self, touch):
        ...
        if y_pos < y_row and x_pos > 50:
            self.menu = False
            new_answers = [random.choice(choice_colors), random.choice(choice_colors), random.choice(choice_colors), random.choice(choice_colors)]
        ...
        for i in range(len(last_turn)):
            if choice_colors[last_turn[i]] == answer_colors[i]:
                ...
                if only_input or several_outputs or count_this:
                    ...
                    response_index += 1
                    print(f'{last_turn[i]} is in the same answer but wrong spot')
